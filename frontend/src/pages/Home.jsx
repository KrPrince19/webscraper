      ...
      {stories.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm">
          ...
          <p className="text-slate-400 mb-6 max-w-md">
            ...
            {!user && " Please login or register to fully explore and bookmark your favorite stories."}
          </p>
          {!user && (
            <div className="flex gap-4">
              <Link to="/login" className="bg-white/10 hover:bg-white/20 text-white py-2 px-6 rounded-full font-medium transition-all">
                Login
              </Link>
              <Link to="/register" className="bg-indigo-500 hover:bg-indigo-600 text-white py-2 px-6 rounded-full font-medium transition-all">
                Register
              </Link>
            </div>
          )}
